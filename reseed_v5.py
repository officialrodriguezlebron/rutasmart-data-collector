"""
RutaSmart reseed v5 — 20 additional trips, May 19–25 + June 1, 3, 4, 8 2026.

Appends only — does NOT delete or modify existing trips or GPS logs.
Follows the same structure as reseed_v4.py.

Trip IDs: JEEP-050 through JEEP-069 (no collision with v4 range).
Both corridors: ~10 MR (MALANDAY-RECTO) + ~10 RM (RECTO-MALANDAY).

Run:
  DATABASE_URL=<railway_url> python reseed_v5.py
"""
...
# ── 20 new trips: May 19–25, Jun 1, 3, 4, 8 ──────────────────────────────────
# Columns: trip_id, direction, (yr,mo,dy,pht_h,pht_m), n_interp, n_dwell, bad_rate
# ~10 MR + ~10 RM; IDs JEEP-050 → JEEP-069 (no collision with v4).

TRIPS_V5 = [
    # May 19 (Tue) ─────────────────────────────────────────────────────────────
    ("JEEP-050", "MALANDAY-RECTO", (2026, 5, 19,  7, 23),  8, 33, 0.15),  # Tue MR  Morning Peak
    ("JEEP-051", "RECTO-MALANDAY", (2026, 5, 19, 17, 44),  6, 38, 0.30),  # Tue RM  Afternoon Peak
    # May 20 (Wed) ─────────────────────────────────────────────────────────────
    ("JEEP-052", "MALANDAY-RECTO", (2026, 5, 20, 16, 11),  9, 35, 0.25),  # Wed MR  Afternoon Peak
    ("JEEP-065", "RECTO-MALANDAY", (2026, 5, 20,  8, 19),  7, 32, 0.14),  # Wed RM  Morning Peak
    # May 21 (Thu) ─────────────────────────────────────────────────────────────
    ("JEEP-053", "RECTO-MALANDAY", (2026, 5, 21,  7, 37),  7, 30, 0.12),  # Thu RM  Morning Peak
    ("JEEP-066", "MALANDAY-RECTO", (2026, 5, 21, 17, 33),  8, 37, 0.32),  # Thu MR  Afternoon Peak
    # May 22 (Fri) ─────────────────────────────────────────────────────────────
    ("JEEP-054", "MALANDAY-RECTO", (2026, 5, 22,  6, 52), 10, 40, 0.08),  # Fri MR  Morning Peak
    ("JEEP-055", "RECTO-MALANDAY", (2026, 5, 22, 16, 29),  7, 36, 0.45),  # Fri RM  Afternoon Peak (degraded)
    # May 23 (Sat) ─────────────────────────────────────────────────────────────
    ("JEEP-056", "MALANDAY-RECTO", (2026, 5, 23, 11, 14),  6, 32, 0.18),  # Sat MR  Midday
    ("JEEP-067", "RECTO-MALANDAY", (2026, 5, 23, 13, 48),  6, 35, 0.19),  # Sat RM  Midday
    # May 25 (Mon) ─────────────────────────────────────────────────────────────
    ("JEEP-057", "MALANDAY-RECTO", (2026, 5, 25,  7,  4),  9, 38, 0.10),  # Mon MR  Morning Peak
    ("JEEP-058", "RECTO-MALANDAY", (2026, 5, 25, 19, 43),  6, 29, 0.22),  # Mon RM  Off-Peak
    # Jun 1 (Mon) ──────────────────────────────────────────────────────────────
    ("JEEP-059", "MALANDAY-RECTO", (2026, 6,  1,  6, 17),  8, 34, 0.16),  # Mon MR  Morning Peak
    ("JEEP-068", "RECTO-MALANDAY", (2026, 6,  1, 18, 14),  5, 31, 0.28),  # Mon RM  Off-Peak
    # Jun 3 (Wed) ──────────────────────────────────────────────────────────────
    ("JEEP-069", "MALANDAY-RECTO", (2026, 6,  3,  8, 47),  9, 43, 0.07),  # Wed MR  Morning Peak
    ("JEEP-060", "RECTO-MALANDAY", (2026, 6,  3, 16, 38),  7, 42, 0.35),  # Wed RM  Afternoon Peak
    # Jun 4 (Thu) ──────────────────────────────────────────────────────────────
    ("JEEP-061", "MALANDAY-RECTO", (2026, 6,  4,  7, 53), 10, 36, 0.05),  # Thu MR  Morning Peak (best)
    ("JEEP-062", "RECTO-MALANDAY", (2026, 6,  4, 12,  7),  7, 33, 0.20),  # Thu RM  Midday
    # Jun 8 (Mon) ──────────────────────────────────────────────────────────────
    ("JEEP-063", "MALANDAY-RECTO", (2026, 6,  8,  7, 28),  9, 44, 0.09),  # Mon MR  Morning Peak
    ("JEEP-064", "RECTO-MALANDAY", (2026, 6,  8, 17, 51),  8, 39, 0.42),  # Mon RM  Afternoon Peak (degraded)
]
...
def main():
    ...
    try:
        ...
        for trip_id, direction, (yr, mo, dy, pht_h, pht_m), n_interp, n_dwell, bad_rate in TRIPS_V5:

            # Skip if trip already exists (idempotent re-runs)
            if db.query(Trip).filter(Trip.trip_id == trip_id).first():
                print(f"  SKIP {trip_id} — already in DB")
                skipped += 1
                continue

            is_return  = direction == "RECTO-MALANDAY"
            ...
            utc_start       = pht_start - timedelta(hours=PHT_OFFSET)
            utc_end         = utc_start + timedelta(seconds=n_pts * LOG_INTERVAL_S)

            trip = Trip(
                trip_id            = trip_id,
                route_id           = "MR-001",
                direction          = direction,
                recorder_id        = f"REC-V5-{trip_id}",
                jeep_code          = trip_id,
                official_capacity  = 26,
                starting_occupancy = random.randint(0, 8),
                status             = TripStatusEnum.COMPLETED,
                start_time         = utc_start,
                end_time           = utc_end,
            )
            db.add(trip)
            db.flush()  # ensure trip_id is visible for FK in GPSLog

            curve_key = (direction, period)
            anchors   = CURVES.get(curve_key, CURVES[(direction, "Off-Peak")])
            ...
            print(f"  {trip_id}")
            print(f"    {direction:<20} {period:<16} {pht_start.strftime('%b %d %H:%M PHT')}")
            print(f"    n_interp={n_interp}  n_dwell={n_dwell}  total={len(logs):,} logs")
            print(f"    GOOD={n_good:,} ({good_pct}%)  ACCEPTABLE={n_acc:,}  POOR={n_poor:,}")
            total_logs += len(logs)

        db.commit()
        inserted = len(TRIPS_V5) - skipped
        print(f"\nDone. {inserted} trips inserted, {skipped} skipped, {total_logs:,} total GPS logs.")
