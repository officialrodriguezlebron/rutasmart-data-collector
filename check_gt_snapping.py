"""
B1 snap-check: report how far each GT stop lies from its nearest corridor point.
Flags stops > 80 m and notes which corridor (MR/RM) gives the closer snap.
Run from project root: python check_gt_snapping.py
"""
import math
import sys
import os

# ── Inline corridor data (mirrors corridor.py) ────────────────────────────────
MR_CORRIDOR = [
...
def snap(lat, lng, corridor):
    bl,bn,bd = lat,lng,math.inf
    for i in range(len(corridor)-1):
        ax,ay=corridor[i]; bx,by=corridor[i+1]
        dx=bx-ax; dy=by-ay; denom=dx*dx+dy*dy
        t=0.0 if denom==0 else max(0.0,min(1.0,((lat-ax)*dx+(lng-ay)*dy)/denom))
        sl=ax+t*dx; sn=ay+t*dy; d=hav(lat,lng,sl,sn)
        if d<bd: bd=d; bl=sl; bn=sn
    return bl,bn,bd

FLAG  = 80.0   # metres
rows  = []

for name, lat, lon in GROUND_TRUTH_STOPS:
    mr_lat,mr_lon,mr_d = snap(lat,lon,MR_CORRIDOR)
    rm_lat,rm_lon,rm_d = snap(lat,lon,RM_CORRIDOR)
    best_d   = min(mr_d, rm_d)
    best_cor = "MR" if mr_d <= rm_d else "RM"
    rows.append((name, lat, lon, round(mr_d,1), round(rm_d,1), round(best_d,1), best_cor))

flagged = [r for r in rows if r[5] > FLAG]
ok      = [r for r in rows if r[5] <= FLAG]

print(f"\n{'='*72}")
print(f"B1 GT SNAP-CHECK   threshold={FLAG}m   stops={len(rows)}   flagged={len(flagged)}")
print(f"{'='*72}")

if not flagged:
    print("All 70 stops are within 80 m of their nearest corridor. No corrections needed.")
else:
    print(f"\n{'#':>3}  {'Stop Name':<42}  {'GT lat':>9}  {'GT lon':>9}  {'MR m':>6}  {'RM m':>6}  {'Best':>6}  {'Corr':>4}  Category")
    print("-"*120)
    for i,(name,lat,lon,mr_d,rm_d,best_d,cor) in enumerate(flagged,1):
        # Classify terminal-loop stops by name heuristic
        is_terminal = any(k in name.lower() for k in ["terminal","malanday","recto lrt","recto ave"])
        cat = "terminal — accept as-is" if is_terminal else "data error — correct"
        print(f"{i:>3}  {name:<42}  {lat:>9.4f}  {lon:>9.4f}  {mr_d:>6}  {rm_d:>6}  {best_d:>6}  {cor:>4}  {cat}")

print(f"\n--- PASS (≤{FLAG}m) ---")
print(f"{'#':>3}  {'Stop Name':<42}  {'MR m':>6}  {'RM m':>6}  {'Best':>6}  Corr")
print("-"*80)
for i,(name,lat,lon,mr_d,rm_d,best_d,cor) in enumerate(ok,1):
    print(f"{i:>3}  {name:<42}  {mr_d:>6}  {rm_d:>6}  {best_d:>6}  {cor}")
print(f"\nDone. {len(ok)} pass / {len(flagged)} flagged.\n")
