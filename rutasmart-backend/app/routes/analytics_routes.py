...
# Used by the Analytics Engine "By Day" and "All Trips" modes to produce
# the same full tab set (Overview, DBSCAN, Load Factor, Demand, Time, Compare)
# as the single-trip mode, but across multiple trips combined.

@router.get("/merged/run-all")
@limiter.limit("5/minute")
def run_merged_analytics(
    request: Request,
    trip_ids: str = Query(..., description="Comma-separated list of trip IDs"),
    eps_m: float  = Query(default=50.0, gt=0, le=500),
    min_samples: int = Query(default=5, ge=2, le=50),
    db: Session = Depends(get_db),
):
    ...
    Returns the same shape as /analytics/{trip_id}/run-all so the
    frontend can reuse all existing tab components unchanged.

    GPS logs from all specified trips are pooled before analysis.
    DBSCAN runs on the combined coordinate set.
    Load factor, demand, and time use the majority capacity.
    """
    ...
    if len(ids) > 50:
        raise HTTPException(status_code=400, detail="Maximum 50 trips per merged analysis.")

    # Fetch all trips and their points
    all_points = []
    capacities = []
    found_ids  = []
