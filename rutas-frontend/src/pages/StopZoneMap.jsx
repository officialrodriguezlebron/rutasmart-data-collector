 ...
 *
 * Road line: hardcoded MacArthur Highway / Rizal Avenue coordinates
 * traced from field knowledge — no routing engine, no OSRM calls.
 * Stop markers: raw cluster centroids — DBSCAN already found road positions.
 * Language: pure passenger framing only.
 */
...
// ── MacArthur Highway → Rizal Avenue: Malanday to Recto ───────────────────
// Traced from field knowledge of the actual jeepney route.
// Key sections:
//   Malanday → local road SE → joins MacArthur Hwy at ~lon 120.969
//   MacArthur Hwy south through Malabon/Caloocan → lon shifts east to ~120.983
//   Around Monumento Circle (west side) → exits south
//   Rizal Avenue south through Manila → Recto LRT
const ROAD = [
  // Malanday terminal → local road southeast
  [14.7187, 120.9575],
  [14.7150, 120.9600],
  [14.7110, 120.9625],
  [14.7075, 120.9648],
  [14.7040, 120.9668],
  // Joins MacArthur Highway (Malabon section)
  [14.7013, 120.9688],
  [14.6975, 120.9712],
  [14.6940, 120.9732],
  // Caloocan / Dagat-dagatan — road curves east
  [14.6905, 120.9752],
  [14.6870, 120.9768],
  [14.6835, 120.9780],
  [14.6800, 120.9792],
  ...
  [14.6574, 120.9836],
  // === Monumento Circle — jeepney goes around WEST side ===
  [14.6568, 120.9830],
  [14.6560, 120.9825],
  [14.6553, 120.9824],
  [14.6548, 120.9827],
  [14.6547, 120.9832],
  [14.6549, 120.9836],
  // Exits Monumento heading south on Rizal Ave
  [14.6540, 120.9836],
  [14.6520, 120.9836],
  [14.6500, 120.9836],
  // Rizal Avenue — Caloocan/Manila boundary
  [14.6475, 120.9834],
  [14.6450, 120.9832],
  [14.6425, 120.9830],
  [14.6400, 120.9828],
...
function PassengerMap({ zones, onClose }) {
  ...
  // Init map once — no routing API calls
  useEffect(() => {
    ...
    // Road line — hardcoded, no routing engine
    L.polyline(ROAD, {
      color: "#42a5f5",
      weight: 4,
      opacity: 0.60,
      ...
      L.marker([la, lo], {
        icon: L.divIcon({
          html: `<div style="
            ...
            border:2px solid #42a5f5;border-radius:8px;
  ...
  useEffect(() => {
    ...
    visible.forEach(zone => {
      ...
      const ring = L.circleMarker([zone.lat, zone.lon], {
        radius: tier.r + 3,
        color: "#ffffff", fillColor: "transparent", fillOpacity: 0,
      ...
      dot.bindTooltip(`
        <div style="font-family:'DM Sans',sans-serif;
          ...
          <div style="font-weight:800;font-size:13px;color:#111;margin-bottom:3px">
            ${tier.badge}${isTop
              ? " <small style='color:#888;font-weight:500'>— Busiest on route</small>"
              : ""}
          </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{
          fontSize: 10, color: "rgba(255,255,255,.28)", marginLeft: "auto",
        }}>Bigger = more reliable</span>
      </div>
      ...
      <div style={{
        ...
        {[
          { color: "#30d158", label: "Frequent stop",   r: 13 },
          { color: "#ffd60a", label: "Regular stop",    r: 9  },
          { color: "#8e9ab0", label: "Occasional stop", r: 6  },
        ].map(({ color, label, r }) => (
        ...
        }}>Stops from real jeepney GPS data</span>
      </div>
    </div>
  );
}
