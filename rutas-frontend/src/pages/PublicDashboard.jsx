      ...
      <div className="pd-header">
        <div className="pd-header-top">
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:8}}>
            ...
            <a href="/" style={{
              fontSize:11,color:"rgba(255,255,255,0.50)",textDecoration:"none",
              fontWeight:600,
            }}>⌂ Home</a>
          </div>
        </div>
      ...
      <div className="pd-content">
        ...
        {staleCount > 0 && (
          <div style={{
            ...
            fontSize:12,color:"#ffd60a",fontWeight:600,
          }}>
            ⚠ {staleCount} jeepney{staleCount>1?"s":""} hidden — no update in over 2 hours
          </div>
        )}

        {/* Where to Board button — always near top */}
        ...
        {/* Empty — no active jeepneys */}
        {data && jeepneys.length === 0 && (
          <div className="pd-empty">
            <span className="pd-empty-icon">🚌</span>
            <h3>No active jeepneys right now</h3>
            <p>No conductor is currently recording a trip on this route.<br/>Data appears here when conductors use the RutaSmart app.</p>
          </div>
        )}
      ...
      <div className="pd-footer">
        <div className="pd-legend">
          {Object.entries(TIER_CONFIG).map(([k,cfg])=>(
            <span key={k} className="pd-legend-item">
              {cfg.emoji} {cfg.label}
            </span>
          ))}
        </div>
